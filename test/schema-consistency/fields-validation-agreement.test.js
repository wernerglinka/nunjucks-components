...
function checkProperty(tree, path, rule) {
  ...
  switch (want) {
    ...
    case 'select':
      return widget === 'select' || widget === 'multiselect'
        ? null
        : { path, message: `enum should be a select, got "${widget}"` };
    case 'number':
      return widget === 'number' ? null : { path, message: `number should use widget "number", got "${widget}"` };
    case 'checkbox':
      return widget === 'checkbox' || widget === 'toggle'
        ? null
        : { path, message: `boolean should be a checkbox, got "${widget}"` };
    default: // text-like
  ...
  for (const section of sections) {
    await t.test(section.name, () => {
      ...
      assert.deepEqual(
        violations,
        [],
        `${section.name} fields/validation widget mismatches:\n  ${violations.join('\n  ')}`
      );
    });
  }
});
