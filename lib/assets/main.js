/**
 * Shell chrome script. header.js only attaches DOMContentLoaded listeners, so its
 * load order relative to this entry does not matter. It lives with the header shell
 * template in layouts/pages/parts rather than the component catalog, and is pulled
 * into the base bundle here because the bundler no longer discovers it as a component.
 */
import '../layouts/pages/parts/header.js';
import '../layouts/pages/parts/navigation.js';

// Optional shell features, toggled by scripts/toggle-feature.mjs. Do not edit tagged lines by hand.
// feature-scripts
import '../layouts/pages/parts/language-switcher.js'; // feature:language-switcher
import '../layouts/pages/parts/dark-light-theme-switcher.js'; // feature:dark-light-theme-switcher
...
function highlightInTextNodeGroup(textNodes, searchTerm) {
  ...
  matches.reverse().forEach((match) => {
    ...
    const involvedNodes = nodeMap.filter((n) => n.start < match.end && n.end > match.start);
...
function initTableWrappers() {
  ...
  tables.forEach((table) => {
    ...
    // Update on window resize; tracked so cleanupMain can remove it when
    // SWUP replaces the page (otherwise each navigation leaks a listener
    // holding the detached scroll container)
    window.addEventListener('resize', updateScrollIndicator);
    tableResizeHandlers.push(updateScrollIndicator);
  });
}

/**
 * Window resize handlers added by initTableWrappers, one per wrapped table
 */
const tableResizeHandlers = [];

/**
 * Cleanup before page transition: highlights UI and per-table resize handlers
 */
function cleanupMain() {
  ...
  tableResizeHandlers.forEach((handler) => {
    window.removeEventListener('resize', handler);
  });
  tableResizeHandlers.length = 0;
}
...
window.PageTransitions.registerCleanup('main', cleanupMain);
