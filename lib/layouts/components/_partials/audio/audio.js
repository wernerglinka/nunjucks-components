...
const initAudioPlayer = (state) => {
  ...
  // Progress bar interactions. Pointer events cover mouse and touch.
  state.progressInput.addEventListener('input', (e) => seekTo(state, e.target.value));
  state.progressInput.addEventListener('pointerdown', () => startDragging(state));
  state.progressInput.addEventListener('pointerup', () => stopDragging(state));
  state.progressInput.addEventListener('pointercancel', () => stopDragging(state));
...
/**
 * Start dragging progress handle. Mutates the shared state so the
 * timeupdate handler stops fighting the user's thumb position.
 * @param {Object} state - Audio player state
 */
const startDragging = (state) => {
  state.isDragging = true;
  state.wasPlayingBeforeDrag = !state.audio.paused;
...
/**
 * Stop dragging progress handle and resume playback if it was playing
 * @param {Object} state - Audio player state
 */
...
const handleKeyboard = (state, e) => {
  // The range inputs (progress, volume) own their arrow keys, and buttons
  // own Enter/Space; shortcuts only apply with focus elsewhere in the player
