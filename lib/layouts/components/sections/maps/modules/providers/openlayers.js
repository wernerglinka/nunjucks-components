 ...
 * Self-hosted copy of the ol npm package's full build, placed under
 * /assets/vendor/ by metalsmith.js; the version is managed in package.json
 * @returns {Promise} Resolves when OpenLayers is loaded
 */
const loadOpenLayers = () => {
  loadStylesheet('/assets/vendor/ol/ol.css');

  return loadScript('/assets/vendor/ol/ol.js', 'ol');
};
...
export const createOpenLayersMap = async (mapContainer) => {
  ...
  const source = clustering.enabled
  ...
  return {
    ...
    cleanup: () => {
      // Detaches the map from its target and removes its listeners; the popup
      // element lives on document.body and must be removed explicitly
      map.setTarget(null);
      popupElement.remove();
