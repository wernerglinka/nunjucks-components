...
function tabKeyFromHash(container) {
  ...
  // The hash is user-controlled; CSS.escape keeps malformed values (dots,
  // colons, leading digits) from throwing inside querySelector
  return container.querySelector(`#tab-${CSS.escape(key)}`) ? key : null;
}
...
function handlePopState() {
  document.querySelectorAll('.tabs-component[data-initialized]').forEach((container) => {
    ...
    const tab = container.querySelector(`#tab-${CSS.escape(key)}`) || container.querySelector('.tabs-button');
    if (tab) {
...
function initMultiTabs() {
  ...
  tabContainers.forEach((container) => {
    ...
    const initialTab = container.querySelector(`#tab-${CSS.escape(initialKey)}`) || tabs[0];
