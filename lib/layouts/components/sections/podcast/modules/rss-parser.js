...
async function parseRSSFeed(rssUrl, options = {}) {
  ...
  try {
    ...
    let channelImageUrl = '';
    const channelImages =
      channel.getElementsByTagNameNS('*', 'image') ||
    ...
    const episodes = items.map((item, index) => {
      ...
      const episodeNumber = getTextContent(item, 'itunes\\:episode') || String(items.length - index).padStart(3, '0');
      // Try to get episode thumbnail using safe namespace handling
      let thumbnail = '';
      let hasEpisodeImage = false;
      const itunesImages =
        item.getElementsByTagNameNS('*', 'image') ||
    ...
    const validEpisodes = episodes.filter((episode) => episode.audioFile);
...
function cleanDescription(description) {
  ...
  // Decode HTML entities first, then strip tags. Stripping before decoding would
  // let encoded markup (&lt;img ...&gt;) survive the strip and re-emerge as live tags.
