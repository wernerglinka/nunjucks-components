...
async function initializePodcastPlayer(section, options = {}) {
    ...
    try {
      ...
      if (episodeListContainer) {
        episodeListContainer.innerHTML =
          '<div class="error-loading">Failed to load episodes. Please try again later.</div>';
      }
      return null;
    }
  }
  ...
  const wrapper = section.querySelector('.podcast-wrapper');
  const podcastTitle =
    podcastHeader?.querySelector('.podcast-title')?.textContent || wrapper?.dataset.podcastTitle || '';
...
/**
 * Escape a value for safe interpolation into HTML element and attribute context.
 * Episode data originates from a remote RSS feed and must never reach innerHTML raw.
...
/**
 * Build the markup for a single episode entry with all feed-derived fields escaped
 * @param {Object} episode - Episode data parsed from the RSS feed
 * @param {boolean} simpleLayout - Whether to use simple layout
 * @returns {string} Episode item HTML
 */
function renderEpisodeItem(episode, simpleLayout = false) {
  const title = escapeHTML(episode.title);
  const duration = escapeHTML(episode.duration);
  const thumbnail = escapeHTML(episode.thumbnail);
  const publishDate = escapeHTML(episode.publishDate);
  const meta = `
              <div class="episode-meta">
                ${episode.publishDate ? `<span class="episode-date">${publishDate}</span>` : ''}
                ${episode.duration ? `<span class="episode-duration">${duration}</span>` : ''}
              </div>`;
  ...
  if (simpleLayout) {
    return `
        <div class="episode-item simple"
             ...
             data-audio-file="${escapeHTML(episode.audioFile)}"
             data-episode-title="${title}"
             data-episode-duration="${duration}"
             data-episode-thumbnail="${thumbnail}">
        ...
        <div class="episode-item"
          ...
          <button class="episode-selector">
            <div class="episode-thumbnail">
              <img src="${thumbnail || '/assets/images/default-podcast-thumb.jpg'}"
                   alt="${title}"
...
function renderEpisodeList(container, episodes, simpleLayout = false) {
  container.innerHTML = episodes.map((episode) => renderEpisodeItem(episode, simpleLayout)).join('');
}
...
function loadMoreEpisodes(container, section, simpleLayout = false) {
  ...
  const newEpisodesHTML = nextBatch.map((episode) => renderEpisodeItem(episode, simpleLayout)).join('');
