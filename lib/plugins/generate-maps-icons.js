 ...
 * @returns {Promise<string|null>} SVG inner content or null if not found
 */
const extractSvgContent = async (iconPath) => {
  try {
...
const generateIconLoaderCode = (iconRegistry) => {
  const registryEntries = Array.from(iconRegistry.entries())
    .map(([name, content]) => {
      // Clean up multiline content and escape for a single-quoted JS string.
      // Backslashes must be escaped first so the quote escapes stay intact.
...
export default function generateMapsIcons() {
  return async (files, metalsmith) => {
    ...
    const iconPromises = Array.from(foundIcons).map(async (iconName) => {
      const iconPath = path.join(iconsDir, `${iconName}.njk`);
      const svgContent = await extractSvgContent(iconPath);

      if (svgContent) {
        return { name: iconName, content: svgContent };
      }
    ...
    results.forEach((result) => {
      if (result) {
        iconRegistry.set(result.name, result.content);
      }
    });
    ...
    await fs.promises.writeFile(outputPath, iconLoaderCode, 'utf8');
  };
}
