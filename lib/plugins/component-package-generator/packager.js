...
export async function createComponentPackage(component, outputPath, generateChecksum) {
  ...
  // Await stream completion here so checksum work below runs outside the
  // event handlers; a rejection there would otherwise hang the promise
  await new Promise((resolve, reject) => {
    ...
    if (examples.raw) {
      archive.append(examples.raw, { name: `${name}/examples.yml` });
    } else if (examples.structured && examples.structured.length > 0) {
      archive.append(yaml.dump(examples.structured), { name: `${name}/examples.yml` });
    } else if (Array.isArray(examples) && examples.length > 0) {
      // Legacy format
      archive.append(yaml.dump(examples), { name: `${name}/examples.yml` });
    }
...
export async function createBundle(components, outputPath, version, generateChecksum) {
  ...
  // Same structure as createComponentPackage: await completion, then hash
  await new Promise((resolve, reject) => {
    output.on('close', resolve);
    archive.on('error', reject);
    output.on('error', reject);
  ...
  const bundleSize = archive.pointer();
...
function addComponentToArchive(archive, component, componentDir) {
  ...
  const examples = component.examples;
  if (examples.raw) {
    archive.append(examples.raw, { name: `${componentDir}/examples.yml` });
  } else if (examples.structured && examples.structured.length > 0) {
    archive.append(yaml.dump(examples.structured), { name: `${componentDir}/examples.yml` });
  } else if (Array.isArray(examples) && examples.length > 0) {
    archive.append(yaml.dump(examples), { name: `${componentDir}/examples.yml` });
  }
