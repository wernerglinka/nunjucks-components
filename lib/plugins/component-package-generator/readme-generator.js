...
export function generateReadme(component) {
  ...
  if (requires.length > 0) {
    ...
    requires.forEach((dep) => {
      readme += `- [${dep}](https://nunjucks-components.com/downloads/partials/${dep}.zip)\n`;
    });
    readme += `\n`;
  ...
  readme += `https://nunjucks-components.com/references/${type === 'section' ? 'sections' : 'partials'}/${name}/\n\n`;
...
export function generateBundleReadme(version, components) {
  ...
  readme += `For complete documentation and live examples, visit:\n`;
  readme += `https://nunjucks-components.com/\n\n`;
